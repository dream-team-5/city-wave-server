...
RSpec.describe Admin::TagsController, type: :controller do
  describe '#resource_model' do
    its(:resource_model) { should eq Tag }
  end

  describe '#resource_params' do
    let(:params) do
      ActionController::Parameters.new tag: { name: 'name' }
    end

    before { allow(subject).to receive(:params).and_return params }

    its(:resource_params) { should eq params[:tag].permit! }
  end

  let(:described_model) { Tag }
